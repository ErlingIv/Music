...
- `sitemap-v2.xml` (Music repo root) — generated, not hand-written. `generate_sitemap.py` (in `E:\OneDrive\database\`, uses `requests` — same sandbox SSL caveat as other scripts, see Python API Pattern note) writes to `OUT_PATH`, which points at this file; queries Supabase for every composition (`score.html?id=`), every person credited on a composition (`composer.html?id=`), every person with `bio_text` (`bio.html?id=`), every tag (`tags.html?tag=`), plus the static landing pages. ~4000 URLs as of August 2026. Re-run it whenever content changes meaningfully — not automated/scheduled yet.
- `sitemap.xml` and `sitemap-v2.xml` both exist and are both live, with identical content, as of August 12, 2026 — this is deliberate, not drift. Added `sitemap-v2.xml` (and repointed `robots.txt`'s `Sitemap:` line and `generate_sitemap.py`'s `OUT_PATH` at it) to test whether Search Console had a stuck/negative processing history tied specifically to the old `sitemap.xml` URL from its initial failed submission on Aug 5, independent of the file's actual validity, which had already been confirmed multiple times via direct `curl`/XML-parse checks (see note below). The old `sitemap.xml` was kept live rather than removed, in case Google's crawler had cached references to individual URLs from it — a first pass at this change did a plain rename and briefly 404'd `sitemap.xml`, corrected the same day by restoring it as a duplicate. `generate_sitemap.py` currently only regenerates `sitemap-v2.xml`; if `sitemap.xml` is kept around longer-term, re-copy it manually after regenerating, or the two will drift out of sync. Revisit whether `sitemap.xml` is still needed once the test's outcome is known.
