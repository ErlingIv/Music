...
- The crop-box drag/resize UI is shared across both entry points via `buildCropBox(wrapEl, imgEl, getBox, setBox)` — box state lives in the caller so each usage stays independent. `mountCropImage(area, imageUrl)` mounts the image itself, sizing it via computed **pixel** `max-width`/`max-height` (from `area.getBoundingClientRect()`) rather than CSS percentages — percentage `max-height` only resolves against an ancestor with a *definite* height, and an auto-sized shrink-wrap div never has one, so that silently became `max-height:none` and let the image overflow its `overflow:hidden` container (visible as the image being stuck showing only its top portion, worst on tall portrait scans, e.g. on iOS). Likewise `cropImageToBlob()` and `uploadAndSetPersonPhoto()` are shared for the canvas-crop and Storage-upload steps. No dedicated field exists yet for "illustrator mark" images distinct from a person's real portrait — `photo_url` is deliberately reused for both, since very few illustrators currently have an actual portrait on file; revisit with a separate `illustrator_mark_url` column if that stops being true.
