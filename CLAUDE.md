...
1. Supabase view/RPC for the grid pages (`musikk-grid.html`, and arguably `index.html`/`composer.html`/`tags.html`) — right now each page independently fetches the full `composition`/`composition_person`/`person` tables and re-implements the same composer/lyricist join + PD/copyright filter in JS. A view exposing `composition_id, title, public_domain, composer_text, lyricist_text, ...` (via `string_agg` per role) queryable the same way as a table (`?public_domain=eq.Yes`) would centralize that and cut redundant transfer. Deliberately keep the "effective nationality" fallback logic (display_country override → composer nationality → mixed-composer-defers-to-lyricist) client-side rather than in the view — it's multi-step conditional logic that's easy to iterate on in JS and awkward in SQL. Not started (August 2026, discussed while reviewing `musikk-grid.html`'s data-fetch pattern).
