...
- Rediger tab → "Noteeksemplar" card: `e_scoreIdLabel` shows the current score_id (`score_id NNNN`) right in the card title — needed to match against staged frontpage filenames (see the frontpage-extraction scripts note above), which the "Forside" upload field below can't otherwise tell you.
