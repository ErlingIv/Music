...
- `sitemap-v2.xml` (Music repo root; named `sitemap.xml` until August 12, 2026 — see dated note below) — generated, not hand-written. `generate_sitemap.py` (in `E:\OneDrive\database\`, uses `requests` — same sandbox SSL caveat as other scripts, see Python API Pattern note) writes to `OUT_PATH`, which points at this file; queries Supabase for every composition (`score.html?id=`), every person credited on a composition (`composer.html?id=`), every person with `bio_text` (`bio.html?id=`), every tag (`tags.html?tag=`), plus the static landing pages. ~4000 URLs as of August 2026. Re-run it whenever content changes meaningfully — not automated/scheduled yet.
- `sitemap.xml` renamed to `sitemap-v2.xml` on August 12, 2026, to test whether Search Console had a stuck/negative processing history tied specifically to the old `sitemap.xml` URL from its initial failed submission on Aug 5 — independent of the file's actual validity, which had already been confirmed multiple times via direct `curl`/XML-parse checks (see note above). `robots.txt`'s `Sitemap:` line and `generate_sitemap.py`'s `OUT_PATH` were both updated to match. The old `sitemap.xml` was deliberately left in place (not deleted) in case Google's crawler had cached references to individual URLs from it — revisit deleting it only once the test's outcome is known.
