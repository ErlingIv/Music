...
- **`submit_indexnow.py`** (in `E:\OneDrive\database\`, not part of the Music git repo — same convention as `generate_sitemap.py`) — submits URLs to `https://api.indexnow.org/indexnow` in batches of 1,000 (IndexNow's own limit is 10,000/request; batching is just a safety margin). Skips records with a null/missing id rather than crashing. Two modes:
  - **Incremental (default)**: `python submit_indexnow.py` — queries `composition`/`person` for `public_content_updated_at >= now - LOOKBACK_DAYS` (2 days), builds `score.html?id=`/`composer.html?id=` URLs from just those rows, **no static pages**. If that combined list is empty, prints `"No new or updated records since last run — skipping IndexNow submission."` and exits 0 without making an HTTP request. Meant to run routinely (e.g. after a batch of editor.js edits) so only what actually changed gets resubmitted.
  - **`--full`**: `python submit_indexnow.py --full` — rebuilds the complete URL list every run (every `composition_id`, every `person_id`, plus the static pages), no date filter, regardless of what actually changed. For the initial submission or a full resync.
